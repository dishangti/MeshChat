...
struct PaymentChipView: View {
    ...
    enum PaymentType {
        ...
        var label: String {
            switch self {
            case .cashu:
                AppLanguageSettings.localized("content.payment.cashu", comment: "Label for Cashu payment chip")
            case .lightning:
                AppLanguageSettings.localized("content.payment.lightning", comment: "Label for Lightning payment chip")
            }
        }
    }
        ...
        .contextMenu {
            if let token = paymentType.cashuToken {
                Button {
                    ...
                    Label(AppLanguageSettings.localized("content.payment.copy_token", comment: "Context menu action copying a Cashu token to the pasteboard"), systemImage: "doc.on.doc")
                }
                Button {
                    redeemCashu()
                } label: {
                    Label(AppLanguageSettings.localized("content.payment.redeem_wallet", comment: "Context menu action opening a Cashu token in an ecash wallet app"), systemImage: "wallet.pass")
                }
                if let webURL = paymentType.cashuWebRedeemURL {
                    Button {
                        openExternalURL(webURL)
                    } label: {
                        Label(AppLanguageSettings.localized("content.payment.redeem_web", comment: "Context menu action opening a Cashu token in the web redemption page"), systemImage: "safari")
                    }
                }
            }
        }
