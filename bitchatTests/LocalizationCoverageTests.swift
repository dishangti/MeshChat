...
struct LocalizationCoverageTests {
    ...
    @Test func englishCoreLabelsAndFormatSpecifiersStayCanonical() throws {
        ...
        let expectedValues = [
            "app_info.about.description": "MeshChat is an open-source fork of BitChat. We thank the BitChat project and all of its contributors for the protocol, security foundation, and original implementation that made MeshChat possible. MeshChat adds a simpler interface and more features while preserving BitChat compatibility.",
            "app_info.appearance.aurora": "Aurora",
            "app_info.appearance.matrix": "Matrix",
            "app_info.appearance.title": "Appearance",
            "app_info.settings.danger.erase_button": "Erase Data",
            "app_info.settings.danger.erase_note": "Deletes chats, media, contacts, nicknames, verification and block records, groups, location history, queued messages, and notification and relay settings. Your identity keys and nickname stay, so your fingerprint does not change. This cannot be undone. Triple-tapping the MeshChat logo also resets identity.",
            "app_info.settings.danger.identity_reset_button": "Reset Identity",
            "app_info.settings.danger.identity_reset_note": "Creates new Mesh, signing, and Nostr keys. Chats, media, contacts, nicknames, blocks, and settings stay. Private groups and queued messages are removed because they use the old keys. Other devices will see you as unverified. This cannot be undone.",
